...
public final class ReedSolomonDecoder {
    ...
    private GenericGFPoly[] runEuclideanAlgorithm(GenericGFPoly genericGFPoly, GenericGFPoly genericGFPoly2, int i) throws ReedSolomonException {
        ...
        while (genericGFPoly.getDegree() >= i / 2) {
            ...
            zero2 = genericGFPoly2;
            while (zero2.getDegree() >= genericGFPoly.getDegree() && !zero2.isZero()) {
                int degree = zero2.getDegree() - genericGFPoly.getDegree();
                ...
                zero2 = zero2.addOrSubtract(genericGFPoly.multiplyByMonomial(degree, multiply));
            }
            zero = genericGFPoly4.multiply(one).addOrSubtract(zero);
            if (zero2.getDegree() >= genericGFPoly.getDegree()) {
        ...
        return new GenericGFPoly[]{one.multiply(coefficient), genericGFPoly.multiply(coefficient)};
    }
