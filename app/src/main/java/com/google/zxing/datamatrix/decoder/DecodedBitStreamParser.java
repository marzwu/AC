...
final class DecodedBitStreamParser {
    ...
    private static Mode decodeAsciiSegment(BitSource bitSource, StringBuilder stringBuilder, StringBuilder stringBuilder2) throws FormatException {
        ...
        do {
            ...
            if (readBits == 0) {
                ...
                stringBuilder.append((char) ((i != 0 ? readBits + 128 : readBits) - 1));
    ...
    private static void decodeBase256Segment(BitSource bitSource, StringBuilder stringBuilder, Collection<byte[]> collection) throws FormatException {
        ...
        if (byteOffset == 0) {
            ...
            available = ((byteOffset - 249) * 250) + unrandomize255State(bitSource.readBits(8), i);
            byteOffset = i + 1;
        }
        if (available < 0) {
            throw FormatException.getFormatInstance();
        ...
        while (i2 < available) {
            ...
            i = byteOffset + 1;
            obj[i2] = (byte) unrandomize255State(bitSource.readBits(8), byteOffset);
            i2++;
            byteOffset = i;
        }
        collection.add(obj);
