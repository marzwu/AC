    ...
    private static int[] findAsteriskPattern(BitArray bitArray, int[] iArr) throws NotFoundException {
        ...
        while (i < size) {
            if ((bitArray.get(i) ^ nextSet) != 0) {
                ...
                if (i3 != length - 1) {
                    i3++;
                } else if (toNarrowWidePattern(iArr) == ASTERISK_ENCODING && bitArray.isRange(Math.max(0, i2 - ((i - i2) / 2)), i2, false)) {
                    return new int[]{i2, i};
                } else {
                    i2 += iArr[0] + iArr[1];
        ...
        if (toNarrowWidePattern == size || i2 * 2 >= i4) {
            if (this.usingCheckDigit) {
                ...
                for (toNarrowWidePattern = 0; toNarrowWidePattern < length; toNarrowWidePattern++) {
                    i2 += ALPHABET_STRING.indexOf(this.decodeRowResult.charAt(toNarrowWidePattern));
                }
                if (obj.charAt(length) != ALPHABET[i2 % 43]) {
            ...
            return new Result(this.extendedMode ? decodeExtended(obj) : obj.toString(), null, new ResultPoint[]{new ResultPoint(((float) (r6[1] + r6[0])) / 2.0f, (float) i), new ResultPoint(f, (float) i)}, BarcodeFormat.CODE_39);
        }
        throw NotFoundException.getNotFoundInstance();
    }
}
